...
# ⚠️ 核心修改区：添加 with_labels，调整节点大小和字体样式
nx.draw(G, pos,
        node_size=120,          # 调大节点尺寸，以便容纳数字
        node_color='#1f77b4',
        edge_color='#cccccc',
        alpha=0.8,
        with_labels=True,       # 开启节点序号显示
        font_size=7,            # 设置字体大小
        font_color='white',     # 设置字体为白色
        font_weight='bold')     # 字体加粗

plt.title(f"Shanghai Bridge KNN Network Topology (K={K_VALUE})", fontsize=16)
plt.axis('equal')
plt.tight_layout()
plt.savefig('graph_visualization.png', dpi=300, bbox_inches='tight')
print("✅ 已保存高清拓扑图片 (带序号): graph_visualization.png")